...
const JoinOrderDialog: React.FC<JoinOrderDialogProps> = ({ product, isOpen, onClose }) => {
  ...
  // Find existing group orders for this product
  const existingOrders = mockGroupOrders.filter(
    order => order.productId === product.id && order.status === 'open'
  );
